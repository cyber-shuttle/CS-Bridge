...
test('csHostAlias is <cluster>-<cs-plane session id>', () => {
    assert.equal(csHostAlias('delta', 's-0123456789ab'), 'delta-s-0123456789ab');
});
...
test('buildSshConfigBlock emits the six SSH resilience options', () => {
    const block = buildSshConfigBlock('sess1', csHostAlias('delta', 's-493119'), 'forward s-493119', 'cs-ssh-user', '/keys/id_cshost-sess1');
    ...
    assert.match(block, /^Host delta-s-493119$/m);
    assert.match(block, /^ {4}ProxyCommand forward s-493119$/m);
...
test('buildSshConfigBlock indents every directive so removeSshConfigEntry can remove it', () => {
    const block = buildSshConfigBlock('s', csHostAlias('delta', 's-abc123'), 'forward', 'u', '/k');
