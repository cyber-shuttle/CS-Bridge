...
// Upsert one on-disk record into mem in place (identity kept). Returns if it changed.
...
// Reconcile in-memory sessions to the full disk set in place (never swap identity, so launch/connect refs stay valid).
...
// Persisted record: the batch script is transient.
export function toPersistedRecord(session: SlurmSession): SlurmSession {
    return { ...session, batchScript: undefined };
}
